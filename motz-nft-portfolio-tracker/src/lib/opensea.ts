...
/**
 * Most recent marketplace sale of a token with the given trait value,
 * where the sale occurred AT OR BEFORE `beforeTs`. Mirrors the Ronin
 * `lastTraitSaleRon` — used to estimate fair-market-value cost basis for
 * ETH-side tokens received via P2P / private transfer.
 *
 * Strategy: walk collection sale events newest-first, stop when we hit
 * a sale before `beforeTs` whose token has the right trait. Cached for
 * FLOOR_TTL_MS per (slug, traitName, tier, dayBucket).
 */
const traitSaleEthCache = new Map<
  string,
  { priceEth: number; ts: number } | null
>();

export async function lastTraitSaleBeforeTs(
  slug: string,
  contract: string,
  traitName: string,
  tier: string,
  ...
  if (traitSaleEthCache.has(ck)) {
    const c = traitSaleEthCache.get(ck);
    return c ? c.priceEth : null;
  }
  ...
  outer: for (let page = 0; page < 30; page++) {
    ...
    let data: Res;
    try {
      data = await osFetch<Res>(`/events/collection/${slug}?${qs.toString()}`);
    } catch {
      break;
    }
    for (const ev of data.asset_events ?? []) {
      ...
      try {
        ...
        if (t === tier) {
          found = {
            priceEth: Number(BigInt(wei)) / 1e18,
            ts: ev.event_timestamp,
          };
          break outer;
        }
  ...
  traitSaleEthCache.set(ck, found);
  return found ? found.priceEth : null;
}
